...
import { CompletionsTypeEnum, HintPathType, QuickMatchMap } from '../interface'
import { extractLastPlaceholder } from '../../utils'

export const hintPlugin = (hintPaths: HintPathType[], quickMatchMap: QuickMatchMap) => {
  return (context: CompletionContext) => {
    ...
    // if (path) {
    //   return {
    //     // from: word.to - latestVarPath.length,
    //     // to: word.to,
    //     from: word.from,
    //     options: [
    //       {label: '1.1', type: 'keyword'},
    //       {label: '1.2', type: 'keyword'},
    //     ]
    //   }
    // }
    // // 如果是}.结尾的，代表是 标签
    if (path.endsWith('}.')) {
      ...
      if (latestVarPath) {
        ...
        // if (data && data.children?.length) {
          ...
          console.log('o', word.to, latestVarPath, path)
          return {
    ...
    if (!path.includes('.') && latestChar !== '.') {
      console.log('ooo', hintPaths?.map?.((item: any) => (
